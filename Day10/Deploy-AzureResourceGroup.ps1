...
Param(
    ...
    [string] $ResourceGroupName = 'TestDay10',
...
[string]$StorageAccountId = New-Guid
$StorageAccountId = $StorageAccountId.Replace('-', '').substring(0, 24)
$OptionalParameters['storageAccountName'] = $StorageAccountId
...
if ($ValidateOnly) {
    ...
    if ($ErrorMessages) {
		...
		$Key = (Get-AzStorageAccountKey -ResourceGroupName $ResourceGroupName -AccountName $StorageAccountId)| Where-Object {$_.KeyName -eq "key1"}

		Write-Output 'Key retrieved'

		$AccountContext = New-AzStorageContext -StorageAccountName $StorageAccountId -StorageAccountKey $Key.Value

		Write-Output 'Account Context Set'

		$AccountContext | Enable-AzStorageStaticWebsite -IndexDocument index.html

		Write-Output 'Static Website Enabled'

		$AccountContext | Set-AzStorageBlobContent -File ./WebsiteFiles/index.html -Container '$web' -Blob 'index.html' -Properties @{"ContentType" = "text/html; charset=utf-8";}

		$AccountContext | Set-AzStorageBlobContent -File ./WebsiteFiles/template.txt -Container '$web' -Blob 'template.txt' -Properties @{"ContentType" = "text/plain; charset=utf-8";}

		Write-Output 'Website Files Copied'
	}
}